 ...
 */
#ifndef TVN_SERVICE_H
#define TVN_SERVICE_H
 ...
 *    "/tmp/tvns_%09i_%09i" which is 36 characters long.
 */
#define TVNS_ADDRLEN 64
 ...
 */
#ifdef TVN_HAVE_TIPC
static inline socklen_t tvns_make_address(tvu_int _service_nr, bool bind, void *_address)
{
...
#endif
#ifndef TVN_HAVE_TIPC
static inline socklen_t tvns_make_address(tvu_int _service_nr, bool bind __attribute__((unused)), void *_address)
{
    ...
    if (_service_nr >= 0) {
        ...
        return offsetof(struct sockaddr_un, sun_path) + sprintf(address->sun_path, "/tmp/tvns_%019llu", (unsigned long long)service_nr) + 1;
    } else {
        address->sun_family = AF_UNIX;
 ...
 * @param address       Address which is filled in.
 */
static inline tvu_int tvns_parse_address(void * restrict _address, socklen_t address_len __attribute__((unused)))
{
#ifdef TVN_HAVE_TIPC
    struct sockaddr_tipc *address = _address;
    ...
    if (memcmp(address->sun_path, "/tmp/tvns_", 17) == 0) {
        sscanf(address->sun_path, "/tmp/tvns_%19llu", &service_nr);
        return service_nr;
        ...
        fprintf(stderr, "Expect either /tmp/tvns_* or /tmp/tvu_client_* addresses.");
        abort();
    }
#endif
}

static inline int tvns_socket(void)
{
#ifdef TVN_HAVE_TIPC
...
static inline int tvns_bind(int socket, tvu_int service_nr)
{
    void                *address = alloca(TVNS_ADDRLEN);
    socklen_t           address_len = tvns_make_address(service_nr, true, address);
...
#ifdef TVN_HAVE_TIPC
static inline int tvns_unbind(int socket, tvu_int service_nr)
{
    void        *address = alloca(TVNS_ADDRLEN);
    socklen_t   address_len = tvns_make_address(service_nr, false, address);

    return bind(socket, (struct sockaddr *)address, address_len);
}
#endif
#ifndef TVN_HAVE_TIPC
static inline int tvns_unbind(int socket __attribute__((unused)), tvu_int service_nr __attribute__((unused)))
{
    return 0;
}
#endif
...
#ifdef TVN_HAVE_TIPC
static inline int tvns_client_bind(int socket, tvu_int *service_nr)
{
    void        *address = alloca(TVNS_ADDRLEN);
    socklen_t   address_len = TVNS_ADDRLEN;
    int         r;
    ...
    *service_nr = tvns_parse_address(address, address_len)
    return 0;
}
#endif
#ifndef TVN_HAVE_TIPC
static inline int tvns_client_bind(int socket, tvu_int *service_nr)
{
    ...
    *service_nr = -((pid << 32) | r);
    return tvns_bind(socket, *service_nr);;
}
#endif

static inline tvu_int tvns_sendto(int socket, tvu_int service_nr, void const * restrict buffer, size_t length, int flags)
{
    void        *address = alloca(TVNS_ADDRLEN);
    socklen_t   address_len = tvns_make_address(service_nr, true, address);
...
static inline tvu_int tvns_recvfrom(int socket, tvu_int * restrict service_nr, void * restrict buffer, size_t length, int flags)
{
    ssize_t     r;
    void        *address = alloca(TVNS_ADDRLEN);
...
static inline int tvns_close(int socket, tvu_int service_nr)
{
    #ifndef TVN_HAVE_TIPC
    void        *address = alloca(TVNS_ADDRLEN);
    socklen_t   address_len;
    address_len = tvns_make_address(service_nr, true, address);
